...
Whenever relative humidity goes above a given threshold (e.g. 80%), the fan turns on. When RH goes below threshold, fan turns off. The EPS32 must be connected to a computer, and you must run the python code. The python code communicates serially with the ESP32, and also shown on the screen a live graph of temperature and relative humidity.
